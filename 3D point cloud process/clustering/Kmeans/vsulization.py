...
if __name__ == "__main__":
    ...
    x = [[randint(0,10000) for _ in range(3)] for _ in range(1000)]
    x = np.array(x)
    ...
    fig,ax = plt.subplots(3,1,subplot_kw={'projection': '3d'})
    fig.suptitle('K - Means')

    
    ax[0].scatter(x[:,0],x[:,1],x[:,2])
    x_up=np.append(x,k_means.centers,axis=0)
    
    color = colors.ListedColormap(color_map[:k])
    ax[1].scatter(x[:,0],x[:,1],x[:,2],c=labels,cmap=color)

    color = colors.ListedColormap(color_map[:k+1])
    labels_up = np.append(labels,[k_means.k_ for _ in range(k_means.k_)])
    ax[2].scatter(x_up[:,0],x_up[:,1],x_up[:,2],c=labels_up,cmap=color)
